...
class gWater(db):
	...
	def insertSql(self):
		return "INSERT INTO gWater VALUES (null, '%s', %s,%s,%s,%s,%s,%s,%s,%s,%s,'%s','%s');" %( _mysql.escape_string(self.owner), self.alkalinity, self.ca,self.mg,self.na,self.co3,self.so4,self.cl,self.testdate,self.profile, _mysql.escape_string(self.description) ,_mysql.escape_string(self.treatmentMethod ))

	def updateSql(self):
		return "UPDATE gWater SET owner='%s', alkalinity= %s,ca=%s,mg=%s,na=%s,co3=%s,so4=%s,cl=%s,testdate=%s,profile=%s, description ='%s',treatmentMethod='%s' WHERE entity=%s);" %( _mysql.escape_string(self.owner), self.alkalinity, self.ca,self.mg,self.na,self.co3,self.so4,self.cl,self.testdate,self.profile, _mysql.escape_string(self.description),_mysql.escape_string(self.treatmentMethod),self.entity )


	def populate(self,row):
		(( self.entity, self.owner,alkalinity,ca,mg,na,co3,so4,cl,self.testdate,self.profile,self.description,self.treatmentMethod),)=row
