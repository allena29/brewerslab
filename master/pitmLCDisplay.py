...
#
    ...
    def decodeMessage(self, data):
        ...
        if not cm.has_key("selfGenerated"):
            cm['selfGenerated'] = False
        if not cm.has_key("autoBlank"):
            cm['autoBlank'] = 0
        if not cm.has_key("doNotScrollToStart"):
            cm['doNotScrollToStart'] = 1
        if not cm.has_key("alignCenter"):
            cm['alignCenter'] = 0
        if not cm.has_key("autoWait"):
            cm['autoWait'] = 0
        if not cm.has_key("cropText"):
            cm['cropText'] = 0
        if not cm.has_key("alert"):
            cm['alert'] = 1
        if not cm.has_key("importance"):
            cm['importance'] = 5
        if cm['line']:
        ...
        if cm['selfGenerated']:
            ...
            self.lcd.scrollText(cm['text'], cm['line'], cm['autoBlank'], cm['doNotScrollToStart'],
                                cm['alignCenter'], cm['autoWait'], cm['cropText'], cm['alert'])
