...
for s in range(5):
	...
	if os.path.exists("ipc/swFerm") and not os.path.exists("ipc/tweet-sg%s" %(sg)):
		...
		for row in cursor:
			...
			flag.close()	
			(recipename,fieldVal)=row
			doTweet(".. measured gravity at %.3f #brewerslab #%s" %(fieldVal,re.compile("[^A-Za-z0-9]").sub('',recipename)))
			print fieldVal,
		print ""
...
if not os.path.exists("ipc/swPump") and  os.path.exists("ipc/swFerm") and not os.path.exists("ipc/tweet-og"):
	...
	if og:
		...
		for row in cursor:
			...
			flag.close()	
			(recipename,fieldVal)=row
			doTweet("%sL of wort in the fermenter - %.3f OG #brewerslab #%s" %(fieldVal,float(og),re.compile("[^A-Za-z0-9]").sub('',recipename)))
...
if os.path.exists("ipc/post-ferm")  and not os.path.exists("ipc/tweet-abv"):
	...
	if fg: 
		...
		for row in cursor:
			...
			flag.close()	
			(recipename,fieldVal)=row
			doTweet("Measured FG as %.3f estimated abv %.2f %% #brewerslab #%s" %(fg,abv,re.compile("[^A-Za-z0-9]").sub('',recipename)))
			print fieldVal,fg,
	print ""
