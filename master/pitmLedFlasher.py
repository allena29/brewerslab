...
class pitmLedFlasher:
	...
	def __init__(self,daemon=False):
		...
		self.flashStatus={'lMash':("noflash",0),'lFerm':("noflash",0),'lBoil':("noflash",0),'lSparge':("noflash",0),'lHlt':("noflash",0),'lSys':("noflash",0)}
	...
	def decodeLedMessage(self,data):
		...
		if cm.has_key("lBuzzer"):	self.doLed('tBuzzer',cm)
		if cm.has_key("lHlt"):	self.doLed( 'lHlt',cm)
		if cm.has_key("lBoil"):	self.doLed( 'lBoil',cm)
		if cm.has_key("lSparge"):	self.doLed('lSparge',cm)
		if cm.has_key("lMash"):	self.doLed('lMash',cm)
		if cm.has_key("lFerm"):	self.doLed('lFerm',cm)
		if cm.has_key("lSys"):	self.doLed('lSys',cm)
