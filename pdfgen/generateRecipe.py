...
# Get Description
cursor=con.cursor()
cursor.execute("select recipeName,description,strapline,export,cosmeticName,estimated_abv,estimated_og,estimated_fg,estimated_ibu,batch_size_required,estimated_ebc,target_mash_temp,fermTemp from gRecipes WHERE recipeName = '%s' ;" %( recipeName ))
for row in cursor:
    (recipe,description,strapline,brewnum,cosmeticRecipeName,ABV,OG,FG,IBU,BATCH,EBC,MASH,FERMTEMP)=row
cursor.close()
